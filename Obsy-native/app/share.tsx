...
/** How many topic shortcuts fit on one row without turning this into a picker. */
const TOPIC_SHORTCUT_COUNT = 3;
...
export default function QuickSaveScreen() {
    ...
    // Newest topics first — a shortcut row, deliberately not a full picker.
    const topicShortcuts = useMemo(
        () => [...topics]
            .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
            .slice(0, TOPIC_SHORTCUT_COUNT),
        [topics],
    );
                    ...
                    <View style={styles.topicRow}>
                        {topicShortcuts.map(topic => {
                            ...
                            return (
                                <TouchableOpacity
                                    key={topic.id}
                                    onPress={() => setSelectedTopicId(active ? null : topic.id)}
                                    style={[
                                        styles.topicChip,
                                        {
                                            borderColor: active ? platformColor : colors.cardBorder,
                                            backgroundColor: active
                                                ? platformColor + (isLight ? '22' : '33')
                                                : 'transparent',
                                        },
                                    ]}
                                >
