# Obsy Terms of Service

**Last Updated:** June 2026

---

## Agreement

By using Obsy ("the App"), you agree to these Terms of Service ("Terms"). If you don't agree, don't use the App. These Terms are an agreement between you and **Daystruct** ("Daystruct", "we", "us"), the operator of Obsy. Together with our Privacy Policy, they serve as the End User License Agreement (EULA) for the App.

> **Published at:** https://www.daystruct.ai/legal/obsy/terms
> This file is the source of truth. When it changes, update the published page to match.

---

## What Obsy Is

Obsy is a visual micro-journal and mood tracker. You capture moments, tag moods, optionally add notes or voice notes, and receive AI-generated reflective insights. It's designed for personal observation and reflection. **It is not therapy, diagnosis, or medical advice.**

---

## Your Account

### Requirements
- You must be at least 13 years old
- You must provide accurate account information
- You're responsible for keeping your login credentials secure
- One account per person (no shared accounts)

### Account Tiers

| Tier | Cost | Features |
|------|------|----------|
| **Free** | Free | Full core features with daily insight limits |
| **Plus** | Subscription | Unlimited insights, insight archives, all AI tones, optional cloud photo backup |

### Subscription Terms (Obsy Plus)

Obsy Plus is an **auto-renewable subscription** sold through the Apple App Store:

- **Price:** $5.99 / month or $49.99 / year (prices may vary by region and are shown in the App before purchase).
- Payment is charged to your **Apple ID** at confirmation of purchase.
- Your subscription **auto-renews** at the same price unless you turn off auto-renew at least **24 hours before** the end of the current period.
- Your Apple ID is charged for renewal within 24 hours before the end of the current period.
- You can manage or cancel your subscription anytime in your device's **App Store** subscription settings. Deleting the App does not cancel your subscription.
- Subscriptions are managed via **RevenueCat** on our side; we never receive or store your payment card details.
- No refunds for partial billing periods, except where required by law or Apple's policies.

---

## Your Content

### Ownership
You own everything you create in Obsy — your photos, journal entries, voice notes, moods, and any content you generate. We don't claim any ownership over your content.

### License to Us
By using Obsy, you grant us a limited license to:
- Store and sync your data to provide the service
- Process your content through our AI providers to generate insights and transcribe voice notes

This license exists solely to operate the App. **We don't use your content for advertising or to train AI models, and we don't sell it.**

### Your Responsibilities
You agree not to:
- Upload illegal content
- Use Obsy to harass, abuse, or harm others
- Attempt to reverse engineer, hack, or compromise the App
- Abuse free-tier limits through automated or fraudulent means

---

## AI-Generated Insights & Transcription

### What They Are
Obsy uses third-party AI providers to generate reflective insights from your moods, journal notes, and (with per-capture consent on Plus) photos:

- **Anthropic (Claude)** — primary provider for insight generation, the Moodverse companion, mood-color generation, and topic-attachment extraction.
- **Google (Gemini)** — fallback for insight generation, and the provider that digests shared links you save.
- **DeepSeek** — Topic Pulse and Mood Signal / Mood Connection interpretation, using non-identifying topic and aggregated mood metadata only.
- **OpenAI (Whisper)** — speech-to-text transcription of voice notes.

AI requests run server-side. See our Privacy Policy for exactly what is shared with each provider.

### What They Aren't
- **Not therapy.** Obsy is not a mental health service.
- **Not medical advice.** Don't use insights for health decisions.
- **Not always accurate.** AI can misinterpret, hallucinate, or produce unexpected results, and transcription may contain errors.

### Your Responsibility
AI insights are for personal reflection only. If you're experiencing mental health challenges, please consult a qualified professional.

---

## Service Availability

### No Guarantees
We strive for high availability but don't guarantee it. The App may be:
- Temporarily unavailable for maintenance
- Affected by factors outside our control (including third-party providers)
- Modified or updated at any time

### Changes to the Service
We may add, modify, or remove features at any time. If we make changes that significantly affect your use, we'll notify you in advance when possible.

---

## Termination

### By You
You can delete your account anytime from settings. This removes your cloud data.

### By Us
We may suspend or terminate your account if you:
- Violate these Terms
- Abuse the service
- Engage in fraudulent activity

We'll provide notice when possible, but reserve the right to act immediately in serious cases.

---

## Disclaimers

### "As Is" Service
Obsy is provided "as is" without warranties of any kind. We don't guarantee uninterrupted service, error-free operation, accurate insights, or that the App will meet your specific needs.

### Limitation of Liability
To the maximum extent permitted by law:
- We're not liable for indirect, incidental, or consequential damages
- Our total liability is limited to the amount you paid us in the past 12 months (or $100, whichever is greater)

This is standard legal protection that lets us offer the service.

---

## Legal Stuff

### Governing Law
These Terms are governed by the laws of the State of Georgia, United States.

### Dispute Resolution
Before filing any lawsuit, you agree to contact us and attempt to resolve the dispute informally for at least 30 days.

### Apple App Store
You acknowledge that these Terms are between you and Daystruct, not Apple, and that Apple is not responsible for the App or its content. Apple is a third-party beneficiary of these Terms and may enforce them.

### Entire Agreement
These Terms, plus our Privacy Policy, constitute the entire agreement between you and Daystruct regarding Obsy.

### Severability
If any part of these Terms is found unenforceable, the rest remains in effect.

---

## Changes to Terms

We may update these Terms as Obsy evolves. When we make significant changes:
- We'll update the "Last Updated" date
- We'll notify you in-app
- Continued use after notice means you accept the new Terms

---

## Contact

Questions about these Terms?

**Email:** legal@daystruct.ai
**Support:** support@daystruct.ai
**Help center:** https://www.daystruct.ai/support/obsy

---

*Thanks for using Obsy. Now go capture something.*
